...
class QueryShardExperiment(val querySource: BufferedSource,
                           val payoffSources: Seq[BufferedSource],
                           val costSources: Seq[BufferedSource],
  ...
  override def iterator: Iterator[QueryData] = {
    ...
    val payoffs = payoffSources map (_.getLines())
    val costs = costSources map (_.getLines())

    new Iterator[QueryData] {

      override def hasNext: Boolean = {
        def allPayoffsHaveNext = payoffs map (_.hasNext) reduce (_ && _)
        def allCostsHaveNext = costs map (_.hasNext) reduce (_ && _)
        if (queries.hasNext && allPayoffsHaveNext && allCostsHaveNext) true
        else {
          def noPayoffHasNext = payoffs map (!_.hasNext) reduce (_ && _)
          def noCostHasNext = costs map (!_.hasNext) reduce (_ && _)
      ...
      override def next(): QueryData = {
        ...
        val nextPayoffs: Seq[List[Double]] = payoffs map (_.next().split(FieldSeparator).toList.map(_.toDouble))
        val nextCosts:   Seq[List[Double]] = costs map (_.next().split(FieldSeparator).toList.map(_.toDouble))
...
object QueryShardExperiment {
  ...
  def fromBasename(basename: String): QueryShardExperiment = {
    ...
    new QueryShardExperiment(
      ...
      for (i <- 0 until shardCount) yield Source.fromFile(s"$basename#$i$PayoffSuffix"),
      for (i <- 0 until shardCount) yield Source.fromFile(s"$basename#$i$CostSuffix"),
      properties
    )
