...
class BucketizeResultsTest extends BaseFunSuite {
  ...
  test("main: all with costs") {
    new FilesWithScores {
      ...
      Lines.fromFile(s"$tmpDir/test#0#0.bucketrankavg").of[Int](_.toInt).toList should contain theSameElementsInOrderAs List(49, 49)
      Lines.fromFile(s"$tmpDir/test#0#1.bucketrankavg").of[Int](_.toInt).toList should contain theSameElementsInOrderAs List(149, 149)
      Lines.fromFile(s"$tmpDir/test#0#2.bucketrankavg").of[Int](_.toInt).toList should contain theSameElementsInOrderAs List(249, 249)
    }
  }
